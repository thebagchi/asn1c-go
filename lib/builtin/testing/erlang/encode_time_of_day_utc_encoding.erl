...
%% -*- erlang -*-
%%
%% Encode a TIME-OF-DAY-UTC-ENCODING SEQUENCE using APER or UPER from
%% Erlang/OTP compiled modules.
%%
%% Usage:
%%   escript encode_time_of_day_utc_encoding.erl -hours <int> -minutes <int> -seconds <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'TIME-OF-DAY-UTC-ENCODING', Hours, Minutes, Seconds},
    case 'TIME-DIFFERENCE':encode('TIME-OF-DAY-UTC-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_time_of_day_utc_encoding.erl -hours <int> -minutes <int> -seconds <int> [-aligned]~n",
        [Msg]),
    halt(1).
