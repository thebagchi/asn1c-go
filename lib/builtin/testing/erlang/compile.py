#!/usr/bin/env python3
"""
Compile the ../*.asn1 reference modules using Erlang asn1ct into aper/ and
uper/ directories.
"""
...
def compile_asn1(module: str, encoding: str, outdir: str) -> bool:
    source = os.path.join(os.pardir, module)  # relative to SCRIPT_DIR
    cmd = [
        "erl",
        "-noshell",
        "-eval",
        f'asn1ct:compile("{source}", [{{outdir, "{outdir}"}}, {encoding}]), halt().',
    ]
