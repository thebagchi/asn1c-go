...
%% -*- erlang -*-
%%
%% Encode a YEAR-DAY-ENCODING SEQUENCE using APER or UPER from Erlang/OTP
%% compiled modules.
%%
%% Usage:
%%   escript encode_year_day_encoding.erl -year <int> -day <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'YEAR-DAY-ENCODING', choice_for(Year), Day},
    case 'YEAR':encode('YEAR-DAY-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_year_day_encoding.erl -year <int> -day <int> [-aligned]~n",
        [Msg]),
    halt(1).
