...
%% -*- erlang -*-
%%
%% Encode a HOURS-AND-FRACTION-ENCODING SEQUENCE using APER or UPER from
%% Erlang/OTP compiled modules.
%%
%% Usage:
%%   escript encode_hours_and_fraction_encoding.erl -hours <int> -fraction <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'HOURS-AND-FRACTION-ENCODING', Hours, Fraction},
    case 'TIME-DIFFERENCE':encode('HOURS-AND-FRACTION-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_hours_and_fraction_encoding.erl -hours <int> -fraction <int> [-aligned]~n",
        [Msg]),
    halt(1).
