...
%% -*- erlang -*-
%%
%% Encode a TIME-OF-DAY-AND-FRACTION-ENCODING SEQUENCE using APER or UPER
%% from Erlang/OTP compiled modules.
%%
%% Usage:
%%   escript encode_time_of_day_and_fraction_encoding.erl -hours <int> -minutes <int> -seconds <int> -fraction <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'TIME-OF-DAY-AND-FRACTION-ENCODING', Hours, Minutes, Seconds, Fraction},
    case 'TIME-DIFFERENCE':encode('TIME-OF-DAY-AND-FRACTION-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_time_of_day_and_fraction_encoding.erl -hours <int> -minutes <int> -seconds <int> -fraction <int> [-aligned]~n",
        [Msg]),
    halt(1).
