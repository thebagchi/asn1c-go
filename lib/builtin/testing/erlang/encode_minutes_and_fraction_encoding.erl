...
%% -*- erlang -*-
%%
%% Encode a MINUTES-AND-FRACTION-ENCODING SEQUENCE using APER or UPER from
%% Erlang/OTP compiled modules.
%%
%% Usage:
%%   escript encode_minutes_and_fraction_encoding.erl -hours <int> -minutes <int> -fraction <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'MINUTES-AND-FRACTION-ENCODING', Hours, Minutes, Fraction},
    case 'TIME-DIFFERENCE':encode('MINUTES-AND-FRACTION-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_minutes_and_fraction_encoding.erl -hours <int> -minutes <int> -fraction <int> [-aligned]~n",
        [Msg]),
    halt(1).
