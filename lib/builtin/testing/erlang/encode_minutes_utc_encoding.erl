...
%% -*- erlang -*-
%%
%% Encode a MINUTES-UTC-ENCODING SEQUENCE using APER or UPER from Erlang/OTP
%% compiled modules.
%%
%% Usage:
%%   escript encode_minutes_utc_encoding.erl -hours <int> -minutes <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'MINUTES-UTC-ENCODING', Hours, Minutes},
    case 'TIME-DIFFERENCE':encode('MINUTES-UTC-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_minutes_utc_encoding.erl -hours <int> -minutes <int> [-aligned]~n",
        [Msg]),
    halt(1).
