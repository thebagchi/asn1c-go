...
%% -*- erlang -*-
%%
%% Encode an HOURS-UTC-ENCODING INTEGER using APER or UPER from Erlang/OTP
%% compiled modules.
%%
%% Usage:
%%   escript encode_hours_utc_encoding.erl -hours <int> [-aligned]
%%
...
main(Args) ->
    ...
    case 'TIME-DIFFERENCE':encode('HOURS-UTC-ENCODING', Hours) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_hours_utc_encoding.erl -hours <int> [-aligned]~n",
        [Msg]),
    halt(1).
