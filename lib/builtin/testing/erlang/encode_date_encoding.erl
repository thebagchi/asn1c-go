...
%% -*- erlang -*-
%%
%% Encode a DATE-ENCODING SEQUENCE using APER or UPER from Erlang/OTP
%% compiled modules.
%%
%% Usage:
%%   escript encode_date_encoding.erl -year <int> -month <int> -day <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'DATE-ENCODING', choice_for(Year), Month, Day},
    case 'YEAR':encode('DATE-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_date_encoding.erl -year <int> -month <int> -day <int> [-aligned]~n",
        [Msg]),
    halt(1).
