...
%% -*- erlang -*-
%%
%% Encode an ANY-CENTURY-ENCODING INTEGER (MIN..MAX) using APER or UPER from
%% Erlang/OTP compiled modules.
%%
%% Usage:
%%   escript encode_any_century_encoding.erl -value <int> [-aligned]
%%
...
main(Args) ->
    ...
    case 'CENTURY':encode('ANY-CENTURY-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_any_century_encoding.erl -value <int> [-aligned]~n",
        [Msg]),
    halt(1).
