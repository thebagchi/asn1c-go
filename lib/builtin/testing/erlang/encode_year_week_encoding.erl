...
%% -*- erlang -*-
%%
%% Encode a YEAR-WEEK-ENCODING SEQUENCE using APER or UPER from Erlang/OTP
%% compiled modules.
%%
%% Usage:
%%   escript encode_year_week_encoding.erl -year <int> -week <int> [-aligned]
%%
...
main(Args) ->
    ...
    Value = {'YEAR-WEEK-ENCODING', choice_for(Year), Week},
    case 'YEAR':encode('YEAR-WEEK-ENCODING', Value) of
        {ok, Enc} ->
...
usage(Msg) ->
    io:format(standard_error,
        "error: ~s~nUsage: escript encode_year_week_encoding.erl -year <int> -week <int> [-aligned]~n",
        [Msg]),
    halt(1).
